...
//	import static com.opengamma.strata.measure.StandardComponents.marketDataFactory;
	...
	public class CalibrationXCcyCheckExample
	{
	  ...
	  /// </summary>
	  private static readonly CurveGroupName CURVE_GROUP_NAME = CurveGroupName.of("USD-EUR-XCCY");
	  /// <summary>
	  ...
	  /// </summary>
	  private static readonly ResourceLocator GROUPS_RESOURCE = ResourceLocator.ofFile(new File(PATH_CONFIG + "curves/groups-xccy.csv"));
	  /// <summary>
	  /// The location of the curve calibration settings file.
	  /// </summary>
	  private static readonly ResourceLocator SETTINGS_RESOURCE = ResourceLocator.ofFile(new File(PATH_CONFIG + "curves/settings-xccy.csv"));
	  /// <summary>
	  /// The location of the curve calibration nodes file.
	  /// </summary>
	  private static readonly ResourceLocator CALIBRATION_RESOURCE = ResourceLocator.ofFile(new File(PATH_CONFIG + "curves/calibrations-xccy.csv"));
	  /// <summary>
	  /// The location of the market quotes file.
	  /// </summary>
	  private static readonly ResourceLocator QUOTES_RESOURCE = ResourceLocator.ofFile(new File(PATH_CONFIG + "quotes/quotes-xccy.csv"));
	  /// <summary>
	  /// The location of the FX rates file.
	  /// </summary>
	  private static readonly ResourceLocator FX_RATES_RESOURCE = ResourceLocator.ofFile(new File(PATH_CONFIG + "quotes/fx-rates-xccy.csv"));
...
//ORIGINAL LINE: com.opengamma.strata.collect.result.Result<?> pv = results.getSecond().getCells().get(i);
	  ...
	  private static Pair<IList<Trade>, Results> calculate(CalculationRunner runner)
	  {
		...
		IDictionary<QuoteId, double> quotes = QuotesCsvLoader.load(VAL_DATE, QUOTES_RESOURCE);
		IDictionary<FxRateId, FxRate> fxRates = FxRatesCsvLoader.load(VAL_DATE, FX_RATES_RESOURCE);

		// create the market data
		MarketData marketData = ImmutableMarketData.builder(VAL_DATE).addValueMap(quotes).addValueMap(fxRates).build();
