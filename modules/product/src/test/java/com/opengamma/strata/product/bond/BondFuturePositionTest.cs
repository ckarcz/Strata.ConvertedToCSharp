...
//	import static org.testng.Assert.assertEquals;
	...
	/// <summary>
	/// Test <seealso cref="BondFuturePosition"/>.
	/// </summary>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public class BondFuturePositionTest
	public class BondFuturePositionTest
	{
	  ...
	  public virtual void test_summarize()
	  {
		...
		PortfolioItemSummary expected = PortfolioItemSummary.builder().id(POSITION_INFO.Id.orElse(null)).portfolioItemType(PortfolioItemType.POSITION).productType(ProductType.BOND_FUTURE).currencies(Currency.USD).description("BondFuture x 10").build();
	  ...
	  public virtual void test_withQuantity()
	  {
		...
		BondFuturePosition expected = BondFuturePosition.builder().info(POSITION_INFO).product(PRODUCT).longQuantity(quantity).build();
	  ...
	  {
		ResolvedBondFutureTrade expected = ResolvedBondFutureTrade.builder().info(POSITION_INFO).product(PRODUCT.resolve(REF_DATA)).quantity(QUANTITY).build();
	  ...
	  //-------------------------------------------------------------------------
	  internal static BondFuturePosition sut()
	  {
		return BondFuturePosition.builder().info(POSITION_INFO).product(PRODUCT).longQuantity(QUANTITY).build();
	  }

	  internal static BondFuturePosition sut2()
	  {
		return BondFuturePosition.builder().info(POSITION_INFO2).product(PRODUCT2).longQuantity(100).shortQuantity(50).build();
	  }
