...
//	import static org.testng.Assert.assertTrue;
	...
	/// <summary>
	/// Test <seealso cref="HullWhiteOneFactorPiecewiseConstantInterestRateModel"/>.
	/// </summary>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public class HullWhiteOneFactorPiecewiseConstantInterestRateModelTest
	public class HullWhiteOneFactorPiecewiseConstantInterestRateModelTest
	{
	  ...
	  private static readonly DoubleArray DCF_FIXED = DoubleArray.of(0.50, 0.48);
	  private static readonly DoubleArray ALPHA_FIXED = DoubleArray.of(0.02, 0.04);
	  private static readonly DoubleArray DCF_IBOR = DoubleArray.of(-1.0, -0.01, 0.01, -0.01, 0.95);
	  private static readonly DoubleArray ALPHA_IBOR = DoubleArray.of(0.00, 0.01, 0.02, 0.03, 0.04);
	  private const double TOLERANCE_RATE = 1.0E-10;
	  private const double TOLERANCE_RATE_DELTA = 1.0E-8;
	  private const double TOLERANCE_RATE_DELTA2 = 1.0E-7;
	  private const double TOLERANCE_ALPHA = 1E-8;
	  private static readonly IborIndex EURIBOR3M = IborIndices.EUR_EURIBOR_3M;
	  ...
	  {
		assertEquals(MEAN_REVERSION, MODEL_PARAMETERS.MeanReversion);
		for (int loopperiod = 0; loopperiod < VOLATILITY.size(); loopperiod++)
		{
		  assertEquals(VOLATILITY.get(loopperiod), MODEL_PARAMETERS.Volatility.get(loopperiod));
		}
		double[] volTime = MODEL_PARAMETERS.VolatilityTime.toArray();
	  ...
	  public virtual void swapRate()
	  {
		...
		{
		  numerator += DCF_IBOR.get(loopcf) * Math.Exp(-ALPHA_IBOR.get(loopcf) * x - 0.5 * ALPHA_IBOR.get(loopcf) * ALPHA_IBOR.get(loopcf));
		}
		double denominator = 0.0;
		for (int loopcf = 0; loopcf < DCF_FIXED.size(); loopcf++)
		{
		  denominator += DCF_FIXED.get(loopcf) * Math.Exp(-ALPHA_FIXED.get(loopcf) * x - 0.5 * ALPHA_FIXED.get(loopcf) * ALPHA_FIXED.get(loopcf));
		}
	  ...
	  public virtual void swapRateDdcf()
	  {
		double shift = 1.0E-8;
		double x = 0.0;
		...
		double[] ddcffComputed = computed.Derivatives.toArray();
		...
		double[] ddcfiComputed = MODEL.swapRateDdcfi1(x, DCF_FIXED, ALPHA_FIXED, DCF_IBOR, ALPHA_IBOR).Derivatives.toArray();
	  ...
	  public virtual void swapRateDx2Ddcf()
	  {
		double shift = 1.0E-7;
		double x = 0.0;
		...
		assertTrue(DoubleArrayMath.fuzzyEquals(dx2DdcffExpected, dx2ddcfComputed.First.toArray(), TOLERANCE_RATE_DELTA2));
		...
		assertTrue(DoubleArrayMath.fuzzyEquals(dx2DdcfiExpected, dx2ddcfComputed.Second.toArray(), TOLERANCE_RATE_DELTA2));
	  }
	  ...
	  public virtual void test_beta()
	  {
		double[] theta = new double[] {0.0, 0.9930234298974474, 1.5013698630136987, 1.9917808219178081, 2.5013698630136987, 2.9972602739726026, 3.5013698630136987, 3.9972602739726026, 4.501220151208923, 4.998487910771765, 5.495890410958904};
		double[] expected = new double[] {0.010526360888642377, 0.008653752074472373, 0.008551601997542554, 0.009479708049949437, 0.009409731278859806, 0.009534948404597303, 0.009504300650429525, 0.009629338816014276, 0.009613195012744198, 0.010403528524805543};
	  ...
	  public virtual void test_lambda()
	  {
