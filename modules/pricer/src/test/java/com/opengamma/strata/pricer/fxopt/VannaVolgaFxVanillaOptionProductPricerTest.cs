...
//	import static org.testng.Assert.assertTrue;
	...
	/// <summary>
	/// Test <seealso cref="VannaVolgaFxVanillaOptionProductPricer"/>.
	/// </summary>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public class VannaVolgaFxVanillaOptionProductPricerTest
	public class VannaVolgaFxVanillaOptionProductPricerTest
	{

	  private static readonly ZoneId ZONE = ZoneId.of("Z");
	  private static readonly ZonedDateTime VAL_DATETIME = ZonedDateTime.of(2011, 6, 13, 13, 10, 0, 0, ZONE);
	  private static readonly LocalDate VAL_DATE = VAL_DATETIME.toLocalDate();
	  private static readonly ZonedDateTime EXPIRY = ZonedDateTime.of(2012, 12, 13, 10, 0, 0, 0, ZONE);
	  private static readonly LocalDate PAY = LocalDate.of(2012, 12, 17);
	  private static readonly ZonedDateTime AFTER = ZonedDateTime.of(2012, 12, 24, 10, 0, 0, 0, ZONE);
	  ...
	  private static readonly ImmutableRatesProvider RATES_PROVIDER_AFTER = RatesProviderFxDataSets.createProviderEurUsdActActIsda(AFTER.toLocalDate());

	  private static readonly DoubleArray TIME_TO_EXPIRY = DoubleArray.of(0.0001, 0.25205479452054796, 0.5013698630136987, 1.0015120892282356, 2.0, 5.001512089228235);
	  private static readonly DoubleArray ATM = DoubleArray.of(0.11, 0.115, 0.12, 0.12, 0.125, 0.13);
	  private static readonly DoubleArray DELTA = DoubleArray.of(0.25);
	  ...
	  private static readonly CurveInterpolator INTERP_STRIKE = CurveInterpolators.DOUBLE_QUADRATIC;
	  private static readonly CurveExtrapolator EXTRAP_STRIKE = CurveExtrapolators.LINEAR;
	  private static readonly InterpolatedStrikeSmileDeltaTermStructure SMILE_TERM = InterpolatedStrikeSmileDeltaTermStructure.of(TIME_TO_EXPIRY, DELTA, ATM, RISK_REVERSAL, STRANGLE, ACT_ACT_ISDA, INTERP_STRIKE, EXTRAP_STRIKE, EXTRAP_STRIKE);
	  private static readonly CurrencyPair CURRENCY_PAIR = CurrencyPair.of(EUR, USD);
	  private static readonly BlackFxOptionSmileVolatilities VOLS = BlackFxOptionSmileVolatilities.of(FxOptionVolatilitiesName.of("Test"), CURRENCY_PAIR, VAL_DATETIME, SMILE_TERM);
	  private static readonly BlackFxOptionSmileVolatilities VOLS_AFTER = BlackFxOptionSmileVolatilities.of(FxOptionVolatilitiesName.of("Test"), CURRENCY_PAIR, AFTER, SMILE_TERM);

	  private const int NB_STRIKES = 11;
	  private const double STRIKE_MIN = 1.00;
	  private const double STRIKE_RANGE = 0.80;
	  private const double NOTIONAL = 100_000_000;
	  private static readonly ResolvedFxVanillaOption[] CALLS = new ResolvedFxVanillaOption[NB_STRIKES];
	  private static readonly ResolvedFxVanillaOption[] PUTS = new ResolvedFxVanillaOption[NB_STRIKES];
	  private static readonly ResolvedFxSingle[] UNDERLYING = new ResolvedFxSingle[NB_STRIKES];
	  ...
	  private static readonly VannaVolgaFxVanillaOptionProductPricer PRICER = VannaVolgaFxVanillaOptionProductPricer.DEFAULT;
	  private static readonly VannaVolgaFxVanillaOptionTradePricer TRADE_PRICER = VannaVolgaFxVanillaOptionTradePricer.DEFAULT;
	  private static readonly DiscountingFxSingleProductPricer FX_PRICER = DiscountingFxSingleProductPricer.DEFAULT;
	  ...
	  public virtual void test_presentValueSensitivity_afterExpiry()
	  {
		for (int i = 0; i < NB_STRIKES; ++i)
		{
		  PointSensitivityBuilder computedCall = PRICER.presentValueSensitivityRatesStickyStrike(CALLS[i], RATES_PROVIDER_AFTER, VOLS_AFTER);
		  assertEquals(computedCall, PointSensitivityBuilder.none());
		  PointSensitivityBuilder computedPut = PRICER.presentValueSensitivityRatesStickyStrike(PUTS[i], RATES_PROVIDER_AFTER, VOLS_AFTER);
	  ...
	  public virtual void test_presentValueSensitivityVolatility_afterExpiry()
	  {
		for (int i = 0; i < NB_STRIKES; ++i)
		{
		  PointSensitivityBuilder computedCall = PRICER.presentValueSensitivityModelParamsVolatility(CALLS[i], RATES_PROVIDER_AFTER, VOLS_AFTER);
		  assertEquals(computedCall, PointSensitivityBuilder.none());
		  PointSensitivityBuilder computedPut = PRICER.presentValueSensitivityModelParamsVolatility(PUTS[i], RATES_PROVIDER_AFTER, VOLS_AFTER);
		  assertEquals(computedPut, PointSensitivityBuilder.none());
		}
	  }
