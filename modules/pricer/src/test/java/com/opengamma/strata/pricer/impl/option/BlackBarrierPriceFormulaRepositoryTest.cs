...
//	import static org.testng.Assert.assertEquals;
	...
	/// <summary>
	/// Test <seealso cref="BlackBarrierPriceFormulaRepository"/>.
	/// </summary>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public class BlackBarrierPriceFormulaRepositoryTest
	public class BlackBarrierPriceFormulaRepositoryTest
	{
	  ...
	  private const double STRIKE_MID = 100;
	  private const double STRIKE_HIGH = 120;
	  private const double STRIKE_LOW = 85;
	  private static readonly double[] STRIKES = new double[] {STRIKE_HIGH, STRIKE_MID, STRIKE_LOW};
	  ...
	  private static readonly double DF_FOR = Math.Exp(-RATE_FOR * EXPIRY_TIME); // 'Base Ccy
	  private static readonly double DF_DOM = Math.Exp(-RATE_DOM * EXPIRY_TIME); // 'Quote Ccy
	  private static readonly double FWD_FX = SPOT * DF_FOR / DF_DOM;
	  ...
	  {
		SimpleConstantContinuousBarrier dwIn = SimpleConstantContinuousBarrier.of(BarrierType.DOWN, KnockType.KNOCK_IN, 0.1d);
		SimpleConstantContinuousBarrier dwOut = SimpleConstantContinuousBarrier.of(BarrierType.DOWN, KnockType.KNOCK_OUT, 0.1d);
		foreach (double strike in STRIKES)
		{
		// call
		  double callVanilla = BlackFormulaRepository.price(FWD_FX, strike, EXPIRY_TIME, VOLATILITY, true) * DF_DOM;
	  ...
	  {
		assertThrowsIllegalArg(() => BARRIER_PRICER.price(BARRIER_UP_IN.BarrierLevel + 0.1, STRIKE_MID, EXPIRY_TIME, COST_OF_CARRY, RATE_DOM, VOLATILITY, true, BARRIER_UP_IN));
		assertThrowsIllegalArg(() => BARRIER_PRICER.price(BARRIER_DOWN_OUT.BarrierLevel - 0.1, STRIKE_MID, EXPIRY_TIME, COST_OF_CARRY, RATE_DOM, VOLATILITY, true, BARRIER_DOWN_OUT));
		assertThrowsIllegalArg(() => BARRIER_PRICER.priceAdjoint(BARRIER_UP_IN.BarrierLevel + 0.1, STRIKE_MID, EXPIRY_TIME, COST_OF_CARRY, RATE_DOM, VOLATILITY, false, BARRIER_UP_IN));
		assertThrowsIllegalArg(() => BARRIER_PRICER.priceAdjoint(BARRIER_DOWN_OUT.BarrierLevel - 0.1, STRIKE_MID, EXPIRY_TIME, COST_OF_CARRY, RATE_DOM, VOLATILITY, false, BARRIER_DOWN_OUT));
	  }
