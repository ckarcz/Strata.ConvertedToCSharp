...
//	import static org.testng.Assert.assertTrue;
	...
	/// <summary>
	/// Test <seealso cref="NormalIborCapletFloorletPeriodPricer"/>.
	/// </summary>
//JAVA TO C# CONVERTER TODO TASK: Most Java annotations will not have direct .NET equivalent attributes:
//ORIGINAL LINE: @Test public class NormalIborCapletFloorletPeriodPricerTest
	public class NormalIborCapletFloorletPeriodPricerTest
	{
	  ...
	  private static readonly NormalIborCapletFloorletExpiryStrikeVolatilities VOLS_ON_FIX = IborCapletFloorletDataSet.createNormalVolatilities(FIXING.atStartOfDay(ZoneOffset.UTC), EUR_EURIBOR_3M);
	  ...
	  private static readonly NormalIborCapletFloorletExpiryStrikeVolatilities VOLS_AFTER_FIX = IborCapletFloorletDataSet.createNormalVolatilities(FIXING.plusWeeks(1).atStartOfDay(ZoneOffset.UTC), EUR_EURIBOR_3M);
	  ...
	  private static readonly NormalIborCapletFloorletExpiryStrikeVolatilities VOLS_AFTER_PAY = IborCapletFloorletDataSet.createNormalVolatilities(DATE_AFTER_PAY.plusWeeks(1).atStartOfDay(ZoneOffset.UTC), EUR_EURIBOR_3M);
	  // Black vols
	  private static readonly BlackIborCapletFloorletExpiryStrikeVolatilities VOLS_NORMAL = IborCapletFloorletDataSet.createBlackVolatilities(VALUATION, EUR_EURIBOR_3M);

	  private const double TOL = 1.0e-14;
	  private const double EPS_FD = 1.0e-6;
	  private static readonly NormalIborCapletFloorletPeriodPricer PRICER = NormalIborCapletFloorletPeriodPricer.DEFAULT;
	  ...
	  public virtual void test_presentValueDelta_formula()
	  {
		...
		double volatility = VOLS.volatility(expiry, STRIKE, forward);
		double df = RATES.discountFactor(EUR, CAPLET_LONG.PaymentDate);
		double expectedCaplet = NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.delta(forward, STRIKE, expiry, volatility, CALL);
		double expectedFloorlet = -NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.delta(forward, STRIKE, expiry, volatility, PUT);
		assertEquals(computedCaplet.Currency, EUR);
		assertEquals(computedCaplet.Amount, expectedCaplet, NOTIONAL * TOL);
		assertEquals(computedFloorlet.Currency, EUR);
		assertEquals(computedFloorlet.Amount, expectedFloorlet, NOTIONAL * TOL);
	  ...
	  public virtual void test_presentValueGamma_formula()
	  {
		...
		double volatility = VOLS.volatility(expiry, STRIKE, forward);
		double df = RATES.discountFactor(EUR, CAPLET_LONG.PaymentDate);
		double expectedCaplet = NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.gamma(forward, STRIKE, expiry, volatility, CALL);
		double expectedFloorlet = -NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.gamma(forward, STRIKE, expiry, volatility, PUT);
		assertEquals(computedCaplet.Currency, EUR);
		assertEquals(computedCaplet.Amount, expectedCaplet, NOTIONAL * TOL);
		assertEquals(computedFloorlet.Currency, EUR);
		assertEquals(computedFloorlet.Amount, expectedFloorlet, NOTIONAL * TOL);
	  ...
	  public virtual void test_presentValueTheta_formula()
	  {
		...
		double volatility = VOLS.volatility(expiry, STRIKE, forward);
		double df = RATES.discountFactor(EUR, CAPLET_LONG.PaymentDate);
		double expectedCaplet = NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.theta(forward, STRIKE, expiry, volatility, CALL);
		double expectedFloorlet = -NOTIONAL * df * CAPLET_LONG.YearFraction * NormalFormulaRepository.theta(forward, STRIKE, expiry, volatility, PUT);
		assertEquals(computedCaplet.Currency, EUR);
		assertEquals(computedCaplet.Amount, expectedCaplet, NOTIONAL * TOL);
		assertEquals(computedFloorlet.Currency, EUR);
		assertEquals(computedFloorlet.Amount, expectedFloorlet, NOTIONAL * TOL);
