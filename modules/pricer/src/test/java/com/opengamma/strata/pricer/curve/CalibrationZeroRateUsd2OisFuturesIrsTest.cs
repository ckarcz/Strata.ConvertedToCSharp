...
//ORIGINAL LINE: @Test public class CalibrationZeroRateUsd2OisFuturesIrsTest
	public class CalibrationZeroRateUsd2OisFuturesIrsTest
	{
	  ...
	  static CalibrationZeroRateUsd2OisFuturesIrsTest()
	  {
	  ...
	  private static readonly DiscountingIborFutureTradePricer FUT_PRICER = DiscountingIborFutureTradePricer.DEFAULT;
...
//ORIGINAL LINE: java.util.Map<com.opengamma.strata.data.MarketDataId<?>, Object> map = new java.util.HashMap<>(ALL_QUOTES.getValues());
	  ...
	  private void assertPresentValue(RatesProvider result)
	  {
		...
		for (int i = 0; i < FWD3_NB_FUT_NODES; i++)
		{
		  CurrencyAmount pvFut = FUT_PRICER.presentValue(((ResolvedIborFutureTrade) fwd3Trades[i + 1]), result, 0.0);
