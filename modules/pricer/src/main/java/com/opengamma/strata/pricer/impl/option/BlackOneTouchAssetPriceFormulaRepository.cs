...
namespace com.opengamma.strata.pricer.impl.option
{
	...
	public class BlackOneTouchAssetPriceFormulaRepository
	{
	  ...
	  /// Small parameter.
	  /// </summary>
	  private const double SMALL = 1.0e-6;
	  ...
	  public virtual double price(double spot, double timeToExpiry, double costOfCarry, double rate, double lognormalVol, SimpleConstantContinuousBarrier barrier)
	  {
		...
		double xE = isKnockIn ? getF(spot, z, lognormalVolT, h, mu, lambda, eta, h) : getE(spot, df1, x2, y2, h, mu, eta);
		return xE;
	  }
	  ...
	  public virtual ValueDerivatives priceAdjoint(double spot, double timeToExpiry, double costOfCarry, double rate, double lognormalVol, SimpleConstantContinuousBarrier barrier)
	  {
		...
		double price = isKnockIn ? getFAdjoint(spot, z, lognormalVolT, h, mu, lambda, eta, h, fDerivFirst, fDerivSecond) : getEAdjoint(spot, df1, x2, y2, h, mu, eta, eDerivFirst, eDerivSecond);
		double zBar = 0.0;
		double y2Bar = 0.0;
		double x2Bar = 0.0;
		double zSqBar = 0.0;
		...
		derivatives[5] += -dxyds * x2Bar / spot + dxyds * y2Bar / spot + dxyds * zBar / spot + dxyds * dxyds * x2SqBar + 2d * dxyds * x2sBar + dxyds * dxyds * y2SqBar - 2d * dxyds * y2sBar + dxyds * dxyds * zSqBar - 2d * dxyds * zsBar;
		return ValueDerivatives.of(price, DoubleArray.ofUnsafe(derivatives));
	  }
	  ...
	  private double getF(double s, double z, double lognormalVolT, double h, double mu, double lambda, double eta, double barrier)
	  {
		...
		return barrier * (Math.Pow(h / s, mu + lambda) * NORMAL.getCDF(eta * z) + Math.Pow(h / s, mu - lambda) * NORMAL.getCDF(eta * (z - 2d * lambda * lognormalVolT)));
	  }
	  ...
	  private double getEAdjoint(double s, double df1, double x, double y, double h, double mu, double eta, double[] firstDerivatives, double[] secondDerivatives)
	  {
		...
		secondDerivatives[0] = -hsMu * n2 * 2d * (mu + 1d) * (2d * mu + 3d) * df1 / s + hsMu * n2 * 4d * (mu + 1d) * df1 / s;
	  ...
	  private double getFAdjoint(double s, double z, double lognormalVolT, double h, double mu, double lambda, double eta, double barrier, double[] firstDerivatives, double[] secondDerivatives)
	  {
		...
		firstDerivatives[0] = barrier * (-(mu + lambda) * hsMuPLa / s * hsMuPLaBar - (mu - lambda) * hsMuMLa / s * hsMuMLaBar); //s
		...
		secondDerivatives[0] = barrier * (hsMuPLa * hsMuPLaBar * (mu + lambda) * (mu + lambda + 1d) / (s * s) + hsMuMLa * hsMuMLaBar * (mu - lambda) * (mu - lambda + 1d) / (s * s));
