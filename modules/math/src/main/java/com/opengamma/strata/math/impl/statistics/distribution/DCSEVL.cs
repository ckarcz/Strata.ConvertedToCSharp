...
namespace com.opengamma.strata.math.impl.statistics.distribution
{
	...
	internal sealed class DCSEVL
	{
	  ...
	  /// <summary>
	  /// Computes the n-term Chebychev series at point 'x' </summary>
	  /// <param name="x"> the position for evaluation </param>
	  /// <param name="cs"> the terms of the Chebychev series </param>
	  /// <param name="n"> the number of terms in the double[] cs </param>
	  /// <returns> the evaluated series </returns>
	  internal static double compute(double x, double[] cs, int n)
	  {
		if (cs == null)
		{
