...
//ORIGINAL LINE: double[][] data = new double[m][m];
	  ...
	  ///  (see <seealso cref="#flattenMatrix"/>) that is, the <b>last</b> index changes most rapidly.  This produces a penalty matrix that acts on a given set of indexes only<P>
	  /// To produce a penalty matrix that acts on multiple indexes, produce one for each set of indexes and add them together (scaling if necessary) </summary>
	  /// <param name="numElements"> The range of each index. In the example above, this would be {n,m} </param>
	  /// <param name="k"> Difference order. Require size[indices] > k </param>
	  /// <param name="index"> Which set of indices does the matrix act on </param>
	  ...
	  /// </para>
	  /// </summary>
	  /// <param name="x"> A non-uniform set of points </param>
	  /// <param name="k"> The order <b>Only first and second order are currently implemented</b> </param>
	  /// <param name="includeEnds"> If true the matrix includes an estimate for the derivative at the end points; if false
	  ///   the first and last rows of the matrix are empty </param>
	  /// <returns> The derivative matrix </returns>
	  public static DoubleMatrix getDerivativeMatrix(double[] x, int k, bool includeEnds)
	  {
		...
		{
		  throw new System.NotSupportedException("cannot handle order (k) > 2");
		}
...
//ORIGINAL LINE: double[][] data = new double[size][size];
	  ...
	  /// of the k^th order derivative estimates at the points.<para>
	  ...
	  /// </summary>
	  /// <param name="x"> A non-uniform set of points </param>
	  /// <param name="k"> order The order <b>Only first and second order are currently implemented</b> </param>
	  /// <returns> The k^th order penalty matrix </returns>
	  public static DoubleMatrix getPenaltyMatrix(double[] x, int k)
	  {
		...
		if (x.Length == 1)
		{
		  ...
		  throw new System.ArgumentException("order too high. Length of x is 1 and k is " + k);
		}
	  ...
	  /// <summary>
	  /// Get a kth order penalty matrix for a non-uniform grid whose values have been flattened to a vector. </summary>
	  /// <param name="x"> the grid positions in each dimension </param>
	  /// <param name="k"> the (finite difference) order </param>
	  /// <param name="index"> which index to act on </param>
	  /// <returns> a penalty matrix  </returns>
	  ...
	  /// is the sum  of penalty matrices that act on each index, scaled by an amount lambda </summary>
	  /// <param name="x"> the grid positions in each dimension </param>
	  /// <param name="k"> the (finite difference) order in each dimension </param>
	  /// <param name="lambda"> the strength of the penalty in each dimension </param>
	  /// <returns> a penalty matrix </returns>
	  ...
	  /// flattened to a vector {A_{0,0}, A_{0,1},...._A_{0,m}, A_{1,0}, A_{1,1},...._A_{1,m},...,A_{n,0}, A_{n,1},...._A_{n,m}}. </summary>
	  /// <param name="aMatrix"> A matrix </param>
	  /// <returns> a the flattened matrix  </returns>
	  public static DoubleArray flattenMatrix(DoubleMatrix aMatrix)
	  {
		...
		{
		  Array.Copy(aMatrix.rowArray(i), 0, data, pos, nCols);
		  pos += nCols;
		}
