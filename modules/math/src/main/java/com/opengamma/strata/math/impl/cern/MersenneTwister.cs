...
namespace com.opengamma.strata.math.impl.cern
{
	...
	public class MersenneTwister : RandomEngine
	{
	  ...
	  private const int MATRIX_A = unchecked((int)0x9908b0df); // constant vector a
	  ...
	  private const int mag0 = 0x0;
	  private const int mag1 = MATRIX_A;
	  //private static final int[] mag01=new int[] {0x0, MATRIX_A};
	  /* mag01[x] = x * MATRIX_A  for x=0,1 */

	  public const int DEFAULT_SEED = 4357;
	  ...
	  /// </summary>
	  protected internal virtual void nextBlock()
	  {
		/*
		// ******************** OPTIMIZED **********************
		// only 5-10% faster ?
		...
		*/
		...
		for (kk = 0; kk < N - M; kk++)
		{
		  ...
		  mt[kk] = mt[kk + M] ^ ((int)((uint)y >> 1)) ^ ((y & 0x1) == 0 ? mag0 : mag1);
		}
		for (; kk < N - 1; kk++)
		{
		  y = (mt[kk] & UPPER_MASK) | (mt[kk + 1] & LOWER_MASK);
		  mt[kk] = mt[kk + (M - N)] ^ ((int)((uint)y >> 1)) ^ ((y & 0x1) == 0 ? mag0 : mag1);
		}
		y = (mt[N - 1] & UPPER_MASK) | (mt[0] & LOWER_MASK);
		mt[N - 1] = mt[M - 1] ^ ((int)((uint)y >> 1)) ^ ((y & 0x1) == 0 ? mag0 : mag1);
	  ...
	  public override int nextInt()
	  {
		...
		int y = mt[mti++];
		y ^= (int)((uint)y >> 11); // y ^= TEMPERING_SHIFT_U(y );
		...
		y ^= (int)((uint)y >> 18); // y ^= TEMPERING_SHIFT_L(y);
	  ...
	  /// This method resets the receiver's entire internal state. </summary>
	  /// <param name="seed"> the seed </param>
	  protected internal virtual int Seed
	  {
		  set
		  {
			mt[0] = value & unchecked((int)0xffffffff);
			for (int i = 1; i < N; i++)
			{
			  mt[i] = (1812433253 * (mt[i - 1] ^ (mt[i - 1] >> 30)) + i);
			  /* See Knuth TAOCP Vol2. 3rd Ed. P.106 for multiplier. */
			  /* In the previous versions, MSBs of the value affect   */
			  /* only MSBs of the array mt[].                        */
			  /* 2002/01/09 modified by Makoto Matsumoto             */
			  mt[i] &= unchecked((int)0xffffffff);
			  /* for >32 bit machines */
			}
			...
			old version was:  
			for (int i = 0; i < N; i++) {
			mt[i] = value & 0xffff0000;
			seed = 69069 * value + 1;
			mt[i] |= (value & 0xffff0000) >>> 16;
			seed = 69069 * value + 1;
			}
			//System.out.println("init done");
			mti = N;
