...
//ORIGINAL LINE: @Test public class ExponentiallyWeightedInterpolationQuantileMethodTest
	public class ExponentiallyWeightedInterpolationQuantileMethodTest
	{
	  ...
	  public virtual void quantile_details()
	  {
		double[] level = new double[] {0.98, 0.981, 0.9811, 0.97};
		for (int i = 0; i < level.Length; i++)
		{
	...
	/* Compare Expected shortfall with numerical integral on the VaR. */
	  ...
	  public virtual void es_extreme()
	  {
		double level = 0.999;
		double es = METHOD.expectedShortfallFromUnsorted(level, DATA_123);
