...
namespace com.opengamma.strata.measure.credit
{
	...
	/// </para>
	/// </summary>
	public class CdsTradeCalculationFunction : CalculationFunction<CdsTrade>
	{

	  /// <summary>
	  /// The calculations by measure.
	  /// </summary>
	  private static readonly ImmutableMap<Measure, SingleMeasureCalculation> CALCULATORS = ImmutableMap.builder<Measure, SingleMeasureCalculation>().put(Measures.PRESENT_VALUE, CdsMeasureCalculations.DEFAULT.presentValue).put(Measures.PV01_CALIBRATED_SUM, CdsMeasureCalculations.DEFAULT.pv01CalibratedSum).put(Measures.PV01_CALIBRATED_BUCKETED, CdsMeasureCalculations.DEFAULT.pv01CalibratedBucketed).put(Measures.PV01_MARKET_QUOTE_SUM, CdsMeasureCalculations.DEFAULT.pv01MarketQuoteSum).put(Measures.PV01_MARKET_QUOTE_BUCKETED, CdsMeasureCalculations.DEFAULT.pv01MarketQuoteBucketed).put(Measures.UNIT_PRICE, CdsMeasureCalculations.DEFAULT.unitPrice).put(CreditMeasures.PRINCIPAL, CdsMeasureCalculations.DEFAULT.principal).put(CreditMeasures.IR01_CALIBRATED_PARALLEL, CdsMeasureCalculations.DEFAULT.ir01CalibratedParallel).put(CreditMeasures.IR01_CALIBRATED_BUCKETED, CdsMeasureCalculations.DEFAULT.ir01CalibratedBucketed).put(CreditMeasures.IR01_MARKET_QUOTE_PARALLEL, CdsMeasureCalculations.DEFAULT.ir01MarketQuoteParallel).put(CreditMeasures.IR01_MARKET_QUOTE_BUCKETED, CdsMeasureCalculations.DEFAULT.ir01MarketQuoteBucketed).put(CreditMeasures.CS01_PARALLEL, CdsMeasureCalculations.DEFAULT.cs01Parallel).put(CreditMeasures.CS01_BUCKETED, CdsMeasureCalculations.DEFAULT.cs01Bucketed).put(CreditMeasures.RECOVERY01, CdsMeasureCalculations.DEFAULT.recovery01).put(CreditMeasures.JUMP_TO_DEFAULT, CdsMeasureCalculations.DEFAULT.jumpToDefault).put(CreditMeasures.EXPECTED_LOSS, CdsMeasureCalculations.DEFAULT.expectedLoss).put(Measures.RESOLVED_TARGET, (rt, smd, rd) => rt).build();
	  ...
	  //-------------------------------------------------------------------------
	  public virtual Type<CdsTrade> targetType()
	  {
	  ...
	  public override Optional<string> identifier(CdsTrade target)
	  {
		return target.Info.Id.map(id => id.ToString());
	  }

	  public virtual Currency naturalCurrency(CdsTrade trade, ReferenceData refData)
	  {
...
//ORIGINAL LINE: private com.opengamma.strata.collect.result.Result<?> calculate(com.opengamma.strata.calc.Measure measure, com.opengamma.strata.product.credit.ResolvedCdsTrade trade, CreditRatesScenarioMarketData marketData, com.opengamma.strata.basics.ReferenceData refData)
	  ...
	  //-------------------------------------------------------------------------
	  delegate object SingleMeasureCalculation(ResolvedCdsTrade trade, CreditRatesScenarioMarketData marketData, ReferenceData refData);
